execute if score game_status value matches 1 as @a[team=player] unless score @s totem_use matches 1 at @s if block ~ ~-1 ~ #floorisair:dead_block run tellraw @a ["",{"translate":"floorisair.text.dead_block","fallback":"%s %s fell to the bottom.","with":[{"translate":"floorisair.title.dead","fallback":"[Dead]","color":"red","bold":true},{"selector":"@s"}]}]
